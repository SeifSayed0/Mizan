...
function DecisionScreen({ decision, onBack }: { decision: typeof decisions[number]; onBack: () => void }) {
  ...
  const handleVote = async (choice: "optionA" | "optionB") => {
    try {
      let realDecisionId = decision.id;
      const uuidPattern = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;

      if (!uuidPattern.test(realDecisionId)) {
        const response = await fetch("/api/mizan/decisions");
        const data = await response.json();

        if (!response.ok || !Array.isArray(data)) {
          throw new Error("تعذر تحميل القرارات");
        }
        ...
        if (!match?.id) {
          throw new Error("القرار الحقيقي غير موجود");
        }

        realDecisionId = match.id;
      }

      const res = await fetch("/api/mizan/votes", {
        method: "POST",
      ...
      setVoted(choice === "optionA" ? "a" : "b");
      toast.success("اتسجل صوتك بشكل مجهول");
    } catch (error) {
      console.error("Vote error:", error);
      toast.error("تعذر تسجيل صوتك، حاول تاني");
    }
  };
